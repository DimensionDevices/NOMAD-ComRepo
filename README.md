...
This repository holds some NOMAD Extensions (.ext), Theme Layouts (.thm) files and example hosted sites, games and apps (.com) files.

Some files are fully functional apps/games while others are more geared towards examples/tech demos. Some were created using LLM's as a quick way of providing many examples for developers to reference and utilise. Those files generated by LLM's have been manually "tweaked" and tested. 

Details for how the .ext, .thm and .com files are created and managed are in the links below.
