...
AUTHOR: NOMAD
DATE: 2026-06-02
DESCRIPTION: An incremental farming game with deep progression
CATEGORY: game

CUSTOMHTML-START
<h1 style="text-align:center">Tap Farmer</h1>
<style>
body{color:white;padding:0 !important}
.tapper-container{max-width:1200px;margin:0 auto;padding:16px}
.tapper-grid{display:grid;grid-template-columns:1fr 1fr;gap:24px}
@media(max-width:700px){.tapper-grid{grid-template-columns:1fr}}
.tapper-stats-card{background:linear-gradient(135deg,#2d2d44,#1a1a2e);border-radius:24px;padding:20px;text-align:center;margin-bottom:16px}
.tapper-stats{font-size:1.8rem;font-weight:bold;color:#facc15}
.tapper-stats small{font-size:0.9rem;color:#a0a0c0}
.tapper-button-wrap{text-align:center;margin:20px 0}
.tapper-main-btn{background:linear-gradient(145deg,#f59e0b,#d97706);border:none;width:200px;height:200px;border-radius:999px;font-size:5rem;cursor:pointer;box-shadow:0 12px 30px rgba(0,0,0,0.3);transition:transform 0.05s linear;touch-action:manipulation}
.tapper-main-btn:active{transform:scale(0.94)}
.tapper-upgrades{background:#1e1e2e;border-radius:24px;padding:20px}
.tapper-upgrades h3{margin:0 0 16px 0;color:#facc15;font-size:1.3rem}
.upgrade-grid{display:flex;flex-direction:column;gap:10px;padding-right:8px}
.upgrade-card{background:#2d2d44;border-radius:16px;padding:12px 16px;cursor:pointer;transition:all 0.08s linear;border:1px solid #3d3d5c}
.upgrade-card:active{transform:scale(0.98);background:#3d3d5c}
.upgrade-card.affordable{border-color:#facc15;box-shadow:0 0 8px rgba(250,204,21,0.3)}
.upgrade-name{font-size:1rem;font-weight:bold;color:#fff;display:flex;justify-content:space-between;align-items:center}
.upgrade-desc{font-size:0.7rem;color:#a0a0c0;margin-top:4px}
.upgrade-cost{color:#facc15;font-weight:bold;font-size:0.85rem}
.upgrade-owned{color:#4ade80;font-size:0.75rem}
.tapper-reset-btn{background:#7f1d1d;color:#fecaca;text-align:center;padding:14px;border-radius:16px;margin-top:16px;cursor:pointer;font-weight:bold}
.tapper-msg{text-align:center;font-size:0.9rem;min-height:40px;color:#facc15;margin:12px 0;padding:8px;background:#1e1e2e;border-radius:16px}
.progress-container{background:#1e1e2e;border-radius:40px;height:20px;margin:12px 0;overflow:hidden}
.progress-bar{background:linear-gradient(90deg,#facc15,#f59e0b);width:0%;height:100%;border-radius:40px;transition:width 0.1s ease;display:flex;align-items:center;justify-content:center;font-size:0.7rem;font-weight:bold;color:#1a1a2e}
.perks-section{background:#1e1e2e;border-radius:24px;padding:20px;margin-top:20px}
.perks-section h3{margin:0 0 12px 0;color:#facc15}
.perk-badge{display:inline-block;background:#2d2d44;padding:6px 12px;border-radius:20px;font-size:0.75rem;margin:4px}
</style>

<div class="tapper-container">
  <div class="tapper-grid">
    <!-- Left column: Stats & Tap Button -->
    <div>
      <div class="tapper-stats-card">
        <div class="tapper-stats" id="tapStats_8x7k2m">🌽 0 crops</div>
        <div class="progress-container">
          <div class="progress-bar" id="tapProgressBar">0%</div>
        </div>
        <div style="display:flex;justify-content:space-between;margin-top:12px;gap:12px">
          <div><small>💪 per tap</small><br><strong id="tapPowerStat">1</strong></div>
          <div><small>🌿 per second</small><br><strong id="tapPassiveStat">0</strong></div>
          <div><small>⭐ critical chance</small><br><strong id="tapCritStat">0%</strong></div>
        </div>
      </div>
      
      <div class="tapper-button-wrap">
        <button id="tapBtn_9w3r5t" class="tapper-main-btn">🌾</button>
      </div>
      
      <div class="tapper-msg" id="tapMsg_2p5r7s">Let's go...!</div>
    </div>
    
    <!-- Right column: Upgrades -->
    <div class="tapper-upgrades">
      <h3>📈 UPGRADES</h3>
      <div class="upgrade-grid" id="tapUpgrades_4h6j8k"></div>
    </div>
  </div>
  
  <!-- Prestige section (unlocks later) -->
  <div class="perks-section" id="perksSection" style="display:none">
    <h3>🌟 PRESTIGE PERKS</h3>
...
let upgrades = {
  ...
  // Prestige helper (unlocks after 1000 total crops)
...
function updateUI() {
  ...
  // Show perks section after 1000 total crops
  ...
  if(prestigeBtn) {
    prestigeBtn.onclick = () => {
      ...
      if(confirm(`Prestige will reset your crops but give you ${pointsGain} points.\nContinue?`)) {
        ...
        // Reset upgrade owned counts and reapply prestige perks
        for(let key in upgrades) {
          let owned = upgrades[key].owned;
          for(let i=0; i<owned; i++) {
            // Reverse effect by resetting stats - simpler to just reset and reapply perks
          }
...
function renderUpgrades() {
  ...
  // Add golden seed only if unlocked
...
// Update passive income display periodically
setInterval(() => {
  updateUI();
